...
function handleUpdateProduct(id) {    
    ...
    inputTitleEdit.value = document.querySelector(`.product-item-${id} h3`).textContent;
    // console.log(title)

    var inputImageEdit = document.querySelector('input[name="imageEdit"]');
    inputImageEdit.value = document.querySelector(`.product-item-${id} img`).textContent;
    
    var inputPriceEdit = document.querySelector('input[name="priceEdit"]');
    inputPriceEdit.value = document.querySelector(`.product-item-${id} span`).textContent;

    var inputDescriptionEdit = document.querySelector('input[name="descriptionEdit"]');
    inputDescriptionEdit.value = document.querySelector(`.product-item-${id} span`).textContent;
    ...
    saveUpdateBtn.onclick = function() {
        ...
        fetch(productsApi + '/' + id, options)
            ...
            .then(function() {
                // getProducts()
                document.querySelector(`.product-item-${id} h3`).textContent = inputTitleEdit.value;
                document.querySelector(`.product-item-${id} img`).textContent = inputImageEdit.value;
                document.querySelector(`.product-item-${id} span`).textContent = inputPriceEdit.value;
                document.querySelector(`.product-item-${id} span`).textContent = inputDescriptionEdit.value;
