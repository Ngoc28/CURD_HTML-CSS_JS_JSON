...
function handleUpdateProduct(id) {    
    ...
    inputTitleEdit.value = document.querySelector(`.product-item${id} .product-name`).value;

    // console.log(title)

    var inputImageEdit = document.querySelector('input[name="imageEdit"]');
    inputImageEdit.value = document.querySelector(`.product-item${id} .product-img`).value;
    
    var inputPriceEdit = document.querySelector('input[name="priceEdit"]');
    inputPriceEdit.value = document.querySelector(`.product-item${id} .product-price`).value;

    var inputDescriptionEdit = document.querySelector('input[name="descriptionEdit"]');
    inputDescriptionEdit.value = document.querySelector(`.product-item${id} .product-description`).value;

    var saveUpdateBtn = document.querySelector('#saveUpdate');
    // console.log(updateBtn)
    saveUpdateBtn.onclick = function() {
        let data = {
            inputTitle: inputTitleEdit.value,
            inputImage: inputImageEdit.value,
            inputPrice: inputPriceEdit.value,
            inputDescription: inputDescriptionEdit.value,
        };
        var options = {
            method: 'PUT',
        ...
        fetch(productsApi + '/' + id, options)
            ...
            .then(function() {
                // getProducts()
                document.querySelector(`.product-item${id} .product-name`).value = inputTitleEdit.value;
                document.querySelector(`.product-item${id} .product-img`).value = inputImageEdit.value
                document.querySelector(`.product-item${id} .product-price`).value = inputPriceEdit.value
                document.querySelector(`.product-item${id} .product-description`).value = inputDescriptionEdit.value
